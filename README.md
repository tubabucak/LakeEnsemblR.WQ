...
`cal_metrics()` returns a nested list by metric and model. For each
metric/model combination, values are stored as data frames with
`datetime` and depth/value columns.
