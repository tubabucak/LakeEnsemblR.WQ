...
#     if(models_coupled[i] == "GLM-AED" | models_coupled[i] == "Simstrat-AED"){
      
#       dict <- LakeEnsemblR_WQ_dictionary[LakeEnsemblR_WQ_dictionary$model == "aed",]
...
#       ## Biogeochemistry
#       # Set models
#       lst[["aed_models"]] <- list(models = c("'aed_noncohesive'",
#                                               "'aed_oxygen'", "'aed_carbon'",
#                                               "'aed_silica'", "'aed_nitrogen'",
#                                               "'aed_phosphorus'", "'aed_organic_matter'", "'aed_totals'"))
      
#       lst[["aed_noncohesive"]] <- list()
...
#       lst[["aed_nitrogen"]] <- list()
      
#       lst[["aed_phosphorus"]] <- list()
      
#       lst[["aed_organic_matter"]] <- list()
...
#         lst[["aed_models"]][["models"]] <- c(lst[["aed_models"]][["models"]],
#                                               "'aed_phytoplankton'")
        
#         num_phyto_groups <- length(names(lst_config[["phytoplankton"]][["groups"]]))
#         lst[["aed_phytoplankton"]] <- list(num_phytos = length(seq_len(num_phyto_groups)),
...
#         lst[["aed_zooplankton"]] <- list(num_zoops = length(seq_len(num_zoop_groups)),
  ...
  if (j == "zooplankton" && any(grepl("\\{prey_num\\}", path))) {
    ...
    if(models_coupled[i] == "GLM-AED2" | models_coupled[i] == "Simstrat-AED2"){
      ...
      lst[["aed2_models"]] <- list(models = c("'aed2_noncohesive'",
                                              ...
                                              "'aed2_phosphorus'", "'aed2_organic_matter'", "'aed2_totals'"))
...
if (!is.null(lst[["aed2_totals"]])) {

  # If vars are missing/NA/empty, set defaults
  if (is.null(lst$aed2_totals$TN_vars) || length(lst$aed2_totals$TN_vars) == 0 ||
      all(is.na(lst$aed2_totals$TN_vars))) {
    lst$aed2_totals$TN_vars <- c("'NIT_nit'","'NIT_amm'","'OGM_don'","'OGM_pon'")
  }

  if (is.null(lst$aed2_totals$TP_vars) || length(lst$aed2_totals$TP_vars) == 0 ||
      all(is.na(lst$aed2_totals$TP_vars))) {
    lst$aed2_totals$TP_vars <- c("'PHS_frp'","'OGM_dop'","'OGM_pop'")
  }

  if (is.null(lst$aed2_totals$TOC_vars) || length(lst$aed2_totals$TOC_vars) == 0 ||
      all(is.na(lst$aed2_totals$TOC_vars))) {
    lst$aed2_totals$TOC_vars <- c("'OGM_doc'","'OGM_poc'")
  }

  # Ensure scales are numeric and non-empty
  if (is.null(lst$aed2_totals$TN_varscale) || length(lst$aed2_totals$TN_varscale) == 0 ||
      all(is.na(lst$aed2_totals$TN_varscale))) {
    lst$aed2_totals$TN_varscale <- c(1, 1, 1, 1)
  } else {
    lst$aed2_totals$TN_varscale <- as.numeric(lst$aed2_totals$TN_varscale)
  }

  if (is.null(lst$aed2_totals$TP_varscale) || length(lst$aed2_totals$TP_varscale) == 0 ||
      all(is.na(lst$aed2_totals$TP_varscale))) {
    lst$aed2_totals$TP_varscale <- c(1, 1, 1)
  } else {
    lst$aed2_totals$TP_varscale <- as.numeric(lst$aed2_totals$TP_varscale)
  }

  if (is.null(lst$aed2_totals$TOC_varscale) || length(lst$aed2_totals$TOC_varscale) == 0 ||
      all(is.na(lst$aed2_totals$TOC_varscale))) {
    lst$aed2_totals$TOC_varscale <- c(1, 1)
  } else {
    lst$aed2_totals$TOC_varscale <- as.numeric(lst$aed2_totals$TOC_varscale)
  }
}
      write_nml(lst, file.path(folder, filename))
    }
