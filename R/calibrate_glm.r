calibrate_glm <- function(var = 'temp',
 ...
 if(file.exists(paste0(path, phyto_file,'-template.csv'))){
   file.copy(paste0(path, phyto_file,'-template.csv'), paste0(path, phyto_file, '.nml'), overwrite = T)
 } 
 }
...
run_glm_optim <- function(p, glmcmd, var, scaling, metric, verbose, calib_setup, path, field_file, phyto_file = phyto_file,
    ...
    if (tools::file_ext(nml_file) == 'csv'){
      ...
      for(i in 1:length(nml_setup$pars[!duplicated(nml_setup$pars)])){
        if (any(nml_setup$pars[!duplicated(nml_setup$pars)][i] == nml_setup$pars[duplicated(nml_setup$pars)])){
          ...
          id = which(eg_nml$p_name == nml_setup$pars[!duplicated(nml_setup$pars)][i])
          eg_nml[id,2:ncol(eg_nml)] = use_p[which(nml_setup$pars[!duplicated(nml_setup$pars)][i] == nml_setup$pars)]
          ...
          id = which(eg_nml$p_name == nml_setup$pars[!duplicated(nml_setup$pars)][i])
          eg_nml[id,2:ncol(eg_nml)] = use_p[!duplicated(nml_setup$pars)][i]
          
          
          # eg_nml <- set_nml(eg_nml,nml_setup$pars[!duplicated(nml_setup$pars)][i],
          #                   use_p[!duplicated(nml_setup$pars)][i])
        }
      }
      write_csv(eg_nml, file = paste0(path, nml_file))
  ...
  for (variable in var){
    ...
    #   nrmse = nrmse * 2 
    # }
    plot(df$observed, df$modeled, main = paste0(variable), col = df$depth)
...
run_glm_optim_parallel <- function(p, glmcmd, var, scaling, metric, verbose, calib_setup, path, field_file, phyto_file = phyto_file,
  ...
  temp_folder = paste0(path, myFun(1), '/')
  ...
  file.copy(from = paste0(path,phyto_file,'.csv'), to = paste0(temp_folder,phyto_file,'.csv'))
  ...
  # file.copy(from = paste0(path,'meteo_file_snowtoPP.csv'), to = paste0(temp_folder,'meteo_file_snowtoPP.csv'))
  ...
  for (nml_file in unique(calib_setup$file)){
    if (tools::file_ext(nml_file) == 'csv'){
      ...
      write_csv(eg_nml, file = paste0(temp_folder, nml_file))
  ...
  # return(error)})
  # 
  if (error != 0 || is.null(error) || is.character(error)){
    fit = 999999
  } else{
    ...
    for (variable in var){
      ...
      #   nrmse = nrmse * 10 
      # }
      plot(df$observed, df$observed, main = paste0(var))
    ...
    Sys.sleep(2)
    file.info(paste0(temp_folder,'/output/output.nc'))
    #link_delete(temp_folder)
    system2(paste0('del -Force ',temp_folder,'output/output.nc'))
    
    system2(paste0('del -Force ',getwd(),'/',temp_folder,'output/output.nc'))
    file.remove(paste0(temp_folder,'/output/output.nc'), recursive = T)
    # dir_delete(paste0(temp_folder,'/output/'))
    unlink(paste0(temp_folder,'/output/'), recursive = T, force = T)
