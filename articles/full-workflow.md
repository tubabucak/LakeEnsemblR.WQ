...
For `output_mode = "raw"` with a depth-resolved variable,
[`plot_sensitivity()`](https://tubabucak.github.io/LakeEnsemblR.WQ/reference/plot_sensitivity.md)
draws one line per parameter value, a shaded band for the 5-95% range
across them, and the median. A thin band means the parameter has little
effect on this variable and is a poor calibration candidate.
[`sensitivity_to_long()`](https://tubabucak.github.io/LakeEnsemblR.WQ/reference/sensitivity_to_long.md)
returns the same data as a tidy data frame if you want to make your own
plot.
